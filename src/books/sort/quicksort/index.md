...
![](./ quick_sort_1.png "figure_1")

//TODO 要能交互
// 改变时间,改变数据

<iframe src="/animation/quick_sort/index.html" height="500px" width="100%">
</iframe>


大纲

0 基本思想,小于的放 左边,大于的放右边,是一种典型的递归分解子问题的解决问题的方式
1. 如何有数据有和key相同的数值,应该怎么办,答案不影响
把问题改成左指针的左边都少于等于key,右边都大于等于key

因为,这也是有一种有序, 当我们不停的缩小有序的区间,最后整个区间都是有序的(严格有序)
也就是当我们的左右指针的移动条件改成`<=,>=`

2. 更好的代码,写更少的字
我们把右边的值放入左边,然后左边的值放入右边,这样交换了两次,

从右边找到第一个小于的值,左边第一个大于的值,然后交换,这样交换了一次
且交换完,依然符合上面的条件:

可以想到最后两个指针相遇的时候,那个值一定是想 < key的,
因为右边先走,
如果停下和左指针相遇,左指针对应的位置一定是 <=key
如果是左指针移动后和右指针想遇,那一定是右指针对应的位置是 <key
...
> left指针保证左边的数都小于等于基准数，right指针保证右边的数都大于等于基准数，当left和right相遇时，将基准数和相遇位置的数交换，此时基准数左边的数都小于等于基准数，基准数右边的数都大于等于基准数，然后递归处理左右两边的数。
...
void quicksort(int l, int r) {
    ...
    a[l] = a[i];
    a[i] = tmp;
    quicksort(l, i-1);//继续处理左边的，这里是一个递归的过程
    quicksort(i+1, r);//继续处理右边的 ，这里是一个递归的过程
}
```
<!-- template end -->
...
![快速排序](./quick_sort_2.png "figure_2")
